...
> 租户流程授权、租户使用记录和业务回调尚未实现。

## 1. 当前模块边界

process 统一承载审批流定义和审批运行，定义层包含：
...
END        按 result_status 把实例置为 APPROVED 或 REJECTED 并记录结束时间
~~~
...
- tenant.process_binding 租户流程授权和 tenant.process_usage_record 使用记录。
- 审批通过后创建业务执行任务，以及 callback 模块的回调执行。
- 运行表的统计汇总表或物化视图。
