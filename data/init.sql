...
-- 节点能力定义不在这个脚本里 seed：内置类型的清单（节点名、图标、配置 Schema 和固定 id）
-- 都在 app/server/process/src/node_catalog.py，应用启动时由
-- service/node_definition_sync.py 写进 process.node_definition。
-- "代码支持哪些节点"因此只有一处答案，加类型不用再回来改这个脚本。
-- 代价是刚建好、还没启动过应用的库这张表是空的，启动一次就补齐（同步是幂等的）。
