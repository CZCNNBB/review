...
-- 节点实例只保存自身能力和配置，不保存下一个节点或分支条件。
CREATE TABLE IF NOT EXISTS process.approval_process_node (
    id UUID PRIMARY KEY,
    ...
    created_at TIMESTAMP WITH TIME ZONE NOT NULL,
    updated_at TIMESTAMP WITH TIME ZONE NOT NULL,
    CONSTRAINT fk_approval_process_node_process
        FOREIGN KEY (process_id) REFERENCES process.approval_process (id),
    CONSTRAINT fk_approval_process_node_node_definition
        FOREIGN KEY (node_definition_id) REFERENCES process.node_definition (id)
);

CREATE INDEX IF NOT EXISTS ix_process_approval_process_node_process_id
    ON process.approval_process_node (process_id);

CREATE INDEX IF NOT EXISTS ix_process_approval_process_node_node_definition_id
    ON process.approval_process_node (node_definition_id);
...
COMMENT ON SCHEMA process IS '审批流定义、节点能力和流程节点实例数据';
...
COMMENT ON TABLE process.approval_process IS '一条完整审批流的当前定义，同时保存审批表单和流程编排';
COMMENT ON COLUMN process.approval_process.id IS '稳定的流程 ID，业务系统发起审批时传入';
COMMENT ON COLUMN process.approval_process.name IS '流程名称';
COMMENT ON COLUMN process.approval_process.description IS '流程说明';
COMMENT ON COLUMN process.approval_process.status IS '流程状态：DRAFT、ENABLED 或 DISABLED';
COMMENT ON COLUMN process.approval_process.form_schema_json IS '审批表单字段、类型、必填和校验规则，采用 JSON Schema 表述';
COMMENT ON COLUMN process.approval_process.form_ui_schema_json IS '表单标签、顺序、分组和展示组件，采用 uiSchema 表述';
COMMENT ON COLUMN process.approval_process.orchestration_json IS '节点关系、分支条件和默认路径';
...
COMMENT ON TABLE process.approval_process_node IS '画布中的具体流程节点实例';
COMMENT ON COLUMN process.approval_process_node.id IS '节点实例主键 ID，由前端拖入画布时生成';
COMMENT ON COLUMN process.approval_process_node.process_id IS '所属流程 ID';
COMMENT ON COLUMN process.approval_process_node.node_definition_id IS '引用的节点能力定义 ID';
COMMENT ON COLUMN process.approval_process_node.name IS '当前节点实例名称';
COMMENT ON COLUMN process.approval_process_node.config_json IS '当前节点实例的全部功能配置，包含审批模式和审批人';
COMMENT ON COLUMN process.approval_process_node.position_json IS '前端画布坐标';
COMMENT ON COLUMN process.approval_process_node.created_at IS '创建时间';
COMMENT ON COLUMN process.approval_process_node.updated_at IS '最后更新时间';
