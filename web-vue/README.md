...
16. **「资源授权」不再是独立一页**，两个授权面板并进了租户详情页（`#/tenants/:id`）。原因：授权本身就是"这个租户能用什么"，而授权接口只有按租户查的，独立成页就得先选租户、再按所有租户扇出；并进详情页后租户已确定，两个请求就够，也不用担心授权了哪个租户。已知代价与旧版一致 —— "这条流程授权给了哪些租户"没有反向视图了（需要时在流程详情页补）。停用/启用授权现在有二次确认，且列表里会标出"资源本身已停用"（授权还在但流程停用了，发起照样失败）。
